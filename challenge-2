...
def is_on_list(code, list):
  return code in list

def add_tracking_code(code, list):
  list.append(code)

def remove_tracking_code(code, list):
  list.remove(code)

def get_tracking_code(pos, list):
  return list[pos-1]
...
def get_the_biggest(list):
  return max(list)

def count_in_list(value, list):
